...
def out_neighbour(u,A,B):
    ...
    # Find the first out-neighbour v2 of vertex u and the weight of arc (u,v2)

    # in the digraph with weighted adjacency matrices A (positive) and B (negative)

    # Used when u has outdegree 1
...
def in_neighbour(u,A,B):
    ...
    # Find the first in-neighbour v1 of vertex u and the weight of arc (v1,u)

    # in the digraph with weighted adjacency matrices A (positive) and B (negative)

    # Used when u has indegree 1
...
def Modify_type3(u,A,B):

    # modify the arc between v1 (in-neighbour of u) & v2 (out-neighbour of u)
    # where u is a type 3 vertex

    v1,p=in_neighbour(u,A,B)

    v2,q=out_neighbour(u,A,B)
...
def Recovery(A1,B1,u,stored):
  
    A=add_vertex(A1,u)

    B=add_vertex(B1,u)

    if stored[0]==1: # vertex u is of type 1
    ...
    elif stored[0]==2: # vertex u is of type 2
        ...
        if p<0:
            ...
            for t in range(len(A)):
                ...
                if t!=v and t!=u:#correction (t!=v) changed to (t!=v and t!=u)

                    A[u][t]=p*B[v][t]

                    B[u][t]=p*A[v][t]

    elif stored[0]==3: # vertex u is of type 3
        ...
        if v1==v2:
            ...
            m1=A[v2][v1] #correction [v1][v2] changed to [v2][v1]

            m2=B[v2][v1] #correction [v1][v2] changed to [v2][v1]
...
def RRR_PTC(A,B,epsilon):

    stored=[] # save type of the deleted vertex, its in-neighbour or out-neighbour,
    # and the weight of the relevant arc(s)
    ...
    while flag==False and u<len(A):
        ...
        if t==1:
            ...
            stored=[1,[v1,p]]  # save type of u, its in-neighbour, and the weight of arc (v1,u)
            ...
            stored=[2,[v2,q]] # save type of u, its out-neighbour, and the weight of arc (u,v2)

            A,B=Reduced_Digraph(A,B,u)

        elif t==3:

            save_vertex_type3 = u # temporarily save u

        if flag: # vertex of type 1 or 2 has been deleted

            A1,B1=RRR_PTC(A,B,epsilon) # Recursively compute TC, using further reduction if possible

            A1,B1=Recovery(A1,B1,u,stored) # Recover TC of the input digraph
    ...
    if u>=len(A):
        ...
        if save_vertex_type3 is not None: # Delete the saved vertex of type3
            ...
            # save type of u, its in-neighbour v1 and the weight of arc (v1,u), and
            # its out-neighbour v2 and the weight of arc (u,v2)

            A,B=Modify_type3(u,A,B) # modify the arc between v1 & v2 

            A,B=Reduced_Digraph(A,B,u) # Delete vertex u
            ...
            A1,B1=boolClosure(n, c, d, arcToIndexMap, arcProbabilityList, epsilon)
...
while option == 1:
    ...
    A,B=getInputMatrices(ch, input1)
